...
from NEMO.views.customization import EmailsCustomization, ToolQualificationCustomization
...
class ToolQualificationTestCase(TestCase):
	...
	def setUp(self):
		self.manager: User = User.objects.create(username='manager', first_name='Facility', last_name='Manager', email="facility.manager@example.com", is_facility_manager=True)
		self.owner: User = User.objects.create(username='mctest', first_name='Testy', last_name='McTester', email="testy.mctester@example.com")
		self.user: User = User.objects.create(username='user', first_name='User', last_name='McTester', email="user.mctester@example.com")
		self.project: Project = Project.objects.create(name="Project 1", application_identifier="P1", account=Account.objects.create(name="Account 1"))
	...
	def test_qualification_expiration_disabled_no_days(self, mock_open, mock_exist):
		...
		ToolQualificationCustomization.set("tool_qualification_expiration_never_used_days", 3)
	...
	def test_qualification_expiration_disabled_no_never_used_days(self, mock_open, mock_exist):
		...
		ToolQualificationCustomization.set("tool_qualification_expiration_days", 3)
		EmailsCustomization.set("user_office_email_address", "user_office@example.com")
		# Trigger the expiration timed service
		do_manage_tool_qualifications()
		# Qualification was NOT removed
	...
	def test_qualification_expired_a_while(self, mock_open, mock_exist):
		...
		ToolQualificationCustomization.set("tool_qualification_expiration_days", 3)
		EmailsCustomization.set("user_office_email_address", "user_office@example.com")
		ToolQualificationCustomization.set("tool_qualification_cc", "qualif_cc@example.com")
		...
		self.assertTrue(EmailLog.objects.filter(sender="user_office@example.com", to__contains=self.user.email).exists())
		self.assertTrue(EmailLog.objects.filter(sender="user_office@example.com", to__contains=prefs.email_alternate).exists())
		# Email was sent to cc address
		self.assertTrue(EmailLog.objects.filter(sender="user_office@example.com", to__contains="qualif_cc@example.com").exists())
	...
	def test_qualification_reminders_wrong_day(self, mock_open, mock_exist):
		...
		ToolQualificationCustomization.set("tool_qualification_reminder_days", 2)
		# Trigger the expiration timed service
		do_manage_tool_qualifications()
		# Qualification was NOT removed (3 days disqualified, but only 2 days since qualification)
		self.assertTrue(Qualification.objects.filter(tool=self.tool, user=self.user).exists())
	...
	def test_qualification_reminders_ok(self, mock_open, mock_exist):
		...
		ToolQualificationCustomization.set("tool_qualification_reminder_days", 1)
		# Trigger the expiration timed service
		do_manage_tool_qualifications()
		# Qualification was NOT removed (3 days disqualified, but only 2 days since qualification)
		self.assertTrue(Qualification.objects.filter(tool=self.tool, user=self.user).exists())
