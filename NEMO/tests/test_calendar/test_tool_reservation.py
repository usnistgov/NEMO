...
class ReservationTestCase(TestCase):
	...
	def test_user_does_not_meet_conditions(self):
		...
		response = self.client.post(reverse('create_reservation'), data, follow=True)
		self.assertEqual(response.status_code, 200)
		self.assertNotContains(response, "You are not qualified to use this tool. Creating, moving, and resizing reservations is forbidden.")
		self.assertContains(response, "You do not belong to any active projects. Thus, you may not create any reservations.")
	...
	def test_reservation_policy_problems(self):
		...
		response = self.client.post(reverse('create_reservation'), data, follow=True)
		self.assertEqual(response.status_code, 200)
		self.assertContains(response, "Your reservation has a duration of 60 minutes. This tool requires a minimum reservation duration of 90 minutes.")
		self.assertContains(response, "Your reservation has a duration of 60 minutes. Reservations for this tool may not exceed 30 minutes.")
		self.assertEqual(Reservation.objects.filter(tool=tool).count(), 0)
		...
		response = self.client.post(reverse('create_reservation'), data, follow=True)
		self.assertEqual(response.status_code, 200)
		self.assertContains(response, "You may only have 1 reservations for this tool per day. Missed reservations are included when counting the number of reservations per day")
		...
		response = self.client.post(reverse('create_reservation'), data, follow=True)
		self.assertEqual(response.status_code, 200)
		self.assertContains(response, "Separate reservations for this tool that belong to you must be at least 120 minutes apart from each other. The proposed reservation ends too close to another reservation.")

		data = self.get_reservation_data(start + timedelta(minutes=30), start + timedelta(minutes=90), tool)
		response = self.client.post(reverse('create_reservation'), data, follow=True)
		self.assertEqual(response.status_code, 200)
		self.assertContains(response, "Separate reservations for this tool that belong to you must be at least 120 minutes apart from each other. The proposed reservation begins too close to another reservation.")
		self.assertContains(response, "Separate reservations for this tool that belong to you must be at least 120 minutes apart from each other. The proposed reservation ends too close to another reservation.")
		...
		response = self.client.post(reverse('create_reservation'), data, follow=True)
		self.assertEqual(response.status_code, 200)
		self.assertContains(response, "You may only reserve up to 90 minutes of time on this tool, starting from the current time onward.")
