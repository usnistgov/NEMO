...
class ItemTree(Widget):
    def render(self, name, value, attrs=None, renderer=None):
        ...
        if area_tree_items:
            ...
            parent_areas_dict = {
                area_tree_item.name: area_tree_item for area_tree_item in model_tree.get_ancestor_areas(area_tree_items)
            }
        ...
        for area in area_tree_items:
            ...
            is_qualified = (
                True
                if not display_all_areas
                else (user and user.is_staff) or (user and area.item in user_accessible_areas)
            )
        ...
        for tool in tools:
            ...
            tool_tree.add(
                ReservationItemType.TOOL,
                tool.category + "/" + tool.name_or_child_in_use_name(parent_ids=tool_parent_ids),
                tool.id,
                is_qualified,
            )
...
class ItemTreeHelper:
    ...
    def __render_helper(self, node, result, category_items_lookup=None):
        ...
        if node.__is_leaf():
            ...
            result += f'<input aria-labelledby="{node.item_type.value}-{node.id}" type="checkbox" id="checkbox-{node.item_type.value}-{node.id}" name="item-checkbox" class="item-checkbox" data-item-id="{node.id}" data-item-type="{node.item_type.value}" style="display: none;">'
            result += f'<a id="{node.item_type.value}-{node.id}" href="javascript:void(0);" onclick="set_selected_item(this, true)" data-item-id="{node.id}" data-item-type="{node.item_type.value}" data-item-name="{node.name}" {css_class}>{node.name}</a>'
